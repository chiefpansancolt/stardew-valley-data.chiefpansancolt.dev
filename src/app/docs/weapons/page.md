...
const topSwords = weapons()
  .swords()
  .sortByDamage('desc')
  .get()
  .slice(0, 5)

topSwords.forEach((w) => console.log(`${w.name}: ${w.damageMin}-${w.damageMax} damage`))
```
...
const earlyWeapons = weapons()
  .byMaxLevel(5)
  .sortByDamage('desc')
  .get()

earlyWeapons.forEach((w) => console.log(`${w.name}: ${w.damageMin}-${w.damageMax}`))
```
